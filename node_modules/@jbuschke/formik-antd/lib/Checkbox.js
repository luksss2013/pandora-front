"use strict";
exports.__esModule = true;
var tslib_1 = require("tslib");
var antd_1 = require("antd");
var formik_1 = require("formik");
var React = tslib_1.__importStar(require("react"));
exports.Checkbox = function (_a) {
    var name = _a.name, validate = _a.validate, restProps = tslib_1.__rest(_a, ["name", "validate"]);
    return (React.createElement(formik_1.Field, { name: name, validate: validate }, function (_a) {
        var _b = _a.field, value = _b.value, onChange = _b.onChange;
        return (React.createElement(antd_1.Checkbox, tslib_1.__assign({ name: name, checked: value, onChange: onChange }, restProps)));
    }));
};
exports.Checkbox.Group = function (_a) {
    var name = _a.name, validate = _a.validate, restProps = tslib_1.__rest(_a, ["name", "validate"]);
    return (React.createElement(formik_1.Field, { name: name, validate: validate }, function (_a) {
        var value = _a.field.value, setFieldValue = _a.form.setFieldValue;
        return (React.createElement(antd_1.Checkbox.Group, tslib_1.__assign({ value: value, onChange: function (v) { return setFieldValue(name, v); } }, restProps)));
    }));
};
//# sourceMappingURL=Checkbox.js.map